...
async function putDB(tableName, key) {
  const params = {
    TableName: tableName,
    Item: {
      ServiceName: key,
...
async function handleRequest(request) {
  const serviceId = "SuccessService";
  ...
  const fetchPromise = new Promise((resolve, reject) => {
    const req = https.request(
      ...
      res.on('data', (data) => {
        ...
        failure = false;
        key = "@SUCCESS_" + serviceName + Date.now();
        body = data.toString();
      })
      ...
      res.on('end', () => {
        if (res.statusCode >= 500) {
          failure = true;
          key = "@SERVICE_FAILURE_" + serviceName + Date.now();
        }
...
async function updateCircuitState(service, serviceId, response) {
  if (
    ...
    await putDB('REQUEST_LOG', response.key);
  }
...
async function logEventStateChange(service, newState) {
  ...
  await putDB("EVENTS", JSON.stringify(stateChangeEntry));
}
